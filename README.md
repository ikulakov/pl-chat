...
### Внутри периметра банка

`.npmrc` по умолчанию настроен на банковский Nexus, снаружи периметра переключается
обратно на публичный npm (инструкция в самом файле).

Ставить pnpm через `corepack` внутри банка нельзя: corepack тянет свой бинарник
напрямую с `registry.npmjs.org` мимо `.npmrc`. Ставим через npm — он уже читает
`.npmrc` рядом:

```bash
npm install -g pnpm@10.34.3   # версия из package.json#packageManager
```

Если после установки `pnpm` не находится, проверьте, что в user PATH каталог
`%APPDATA%\npm` записан абсолютным путём: Windows не разворачивает `%USERPROFILE%`,
если значение PATH сохранено как `REG_SZ`.
