...
/**
 * Снаружи в слайс components/ ходят только через его index.ts. Слайсы плоские — все лежат
 * папками первого уровня в components/, друг в друга не вкладываются, — и index.ts —
 * единственное, что слайс обещает соседям; остальное можно переставлять, не трогая чужой код.
 * Без линта баррели расползаются: соседу проще взять файл напрямую, и через месяц index.ts врёт.
 *
...
export default tseslint.config(
  ...
  ...tseslint.configs.recommended,
  {
    ...
    rules: {
      ...
      'no-restricted-syntax': [
        ...
        // них (фича-флаги — в shared/constants/features.ts). Односегментными остаются только слои с
        ...
        // берутся относительным путём, как соседи по корню.
        {
          ...
          message:
            "Абсолютный импорт указывает на слой: '@/shared/constants/features', а не '@/features'. Файл в корне src берётся относительным путём.",
        },
  ...
  //
  // `i18n` из запрета исключён сознательно, это не недосмотр: к нему ходят форматтеры
  // (`formatSize`, `formatDate`) — «2,4 МБ» без локали не напечатать, и i18n тут не верхний
  // слой, а инфраструктура-ровесник.
  //
  // `domain` запрещён с 2026-09-21. До того к `domain/emoji` ходила подсистема анимации эмодзи
  // и стикеров: она лежала в shared/ по единственному признаку «нужна двоим» — тем самым путём,
  // которым shared/ превращается в свалку. Её разложили по потребителям (components/Emoji/lottie,
  // components/Emoji/cache, matrix/catalog/animationBatcher), и запрет вернулся.
  {
