...
## 6. Когда грузится виджет

По умолчанию бандл виджета (~106 КБ) качается **после события `load` вашей страницы**, в
первом простое браузера (а если `load` задерживается дольше 5 секунд — по этому потолку:
на реальных страницах один зависший сторонний ресурс может не дать `load` наступить вовсе) — открытие при этом мгновенное, а загрузка не отбирает полосу у
ваших собственных ресурсов. Менять это не нужно; опция есть на случай нестандартных
страниц:

```js
ChatSDK.init({
  chatUrl: 'https://chat.bank.ru',
  preload: 'idle', // по умолчанию
})
```

| `preload`             | Когда качается виджет                   | Первое открытие                                                                 |
| --------------------- | --------------------------------------- | ------------------------------------------------------------------------------- |
| `idle` (по умолчанию) | после `load` (не дольше 5 с), в простое | мгновенно                                                                       |
| `eager`               | сразу на `init()`                       | мгновенно                                                                       |
| `on-open`             | по первому `open()`                     | с задержкой на загрузку: ~0,3 с на LTE, ~1,2 с на медленном мобильном интернете |

`on-open` берите только там, где чат открывают единицы процентов посетителей и трафик
остальных важнее скорости первого открытия. Повторные визиты в любом режиме почти
бесплатны — статика отдаётся с `immutable`.
